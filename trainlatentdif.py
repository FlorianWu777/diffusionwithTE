...
def evaluate_on_validation(valid_loader, encoder, unet, schedule, device):
   ...
   for x, y in valid_loader:
       ...
       pred = unet(x_t, timestep=t, encoder_hidden_states=cond_feat)
...
def train_loop(dataloader, valid_loader, encoder, unet, optimizer, schedule, device, writer, epochs, desc, save_path):
   ...
   for epoch in range(epochs):
       ...
       encoder.train()
       unet.train()
       for x, y in tqdm(dataloader, desc=f"[{desc}] Epoch {epoch+1}", disable=dist.get_rank() != 0):
           x, y = x.to(device), y.to(device)
           B, V, T_in, H, W = x.shape
           T_out = y.shape[1]
...
#    device = torch.device(f"cuda:{local_rank}")
   ...
   cmip_loader = DataLoader(cmip_dataset, batch_size=16, sampler=cmip_sampler, num_workers=8)
   ...
   # ´´½¨ DataLoader
   reanal_train_loader = DataLoader(reanal_train_set, batch_size=16, sampler=reanal_train_sampler, num_workers=4)
   reanal_valid_loader = DataLoader(reanal_valid_set, batch_size=16, sampler=reanal_valid_sampler, num_workers=4)
   reanal_test_loader = DataLoader(reanal_test_set, batch_size=16, sampler=reanal_test_sampler, num_workers=4)

   # Pretrain
   train_loop(cmip_loader, valid_loader=reanal_valid_loader,encoder, unet, optimizer, schedule, device, writer, epochs=100, desc="Pretrain")
   if local_rank == 0:
       torch.save({'encoder': encoder.module.state_dict(), 'unet': unet.module.state_dict()}, 'swin_unet_pretrained.pt')

   # Finetune
   train_loop(reanal_train_loader,valid_loader=reanal_valid_loader, encoder, unet, optimizer, schedule, device, writer, epochs=300, desc="Finetune")
   if local_rank == 0:
