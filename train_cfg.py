...
drop_prob = 0.1      # classifier-free dropout概率
lambda_hist = 0.1    # 直方图损失权重
mix_prob = 0.5       # 从 x_T 随机生成 vs q_sample 的混合比例

for epoch in range(EPOCHS_FINETUNE):
    model.train()
    running_loss = 0

    for x, y in tqdm(reanal_dataloader):
        x, y = x.to(device), y.to(device)  # x: [B, V, T, H, W], y: [B, T_out, H, W]
        B, V, T_in, H, W = x.shape
        ...
        if torch.rand(1).item() < drop_prob:
            x_cond = torch.zeros_like(x_cond)
        ...
        if torch.rand(1).item() < mix_prob:
            ...
            x_start = y
            x_t = q_sample(x_start, t, noise, schedule)
            supervise_mode = 'noise'
        else:
            # 模式 2：从纯噪声 x_T 开始，预测 x_0
        ...
        # === 损失 ===
        if supervise_mode == 'noise':
            loss_denoise = F.mse_loss(pred_noise, noise)
        else:
            loss_denoise = F.mse_loss(x_pred, x_start)
