...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1.,
    ...
    def model_fn(x, t_continuous):
        ...
        latent = model_kwargs.get("latent", None)
        return model(x, t_discrete, latent=latent, **model_kwargs)
...
class SeaIceForecastPipeline:
    def __init__(self, model, device='cuda', schedule_type='cosine', in_chans=24):
        self.device = device
        ...
        # 创建噪声调度器（VP SDE）
        self.noise_schedule = NoiseScheduleVP(schedule=schedule_type)

        # 包装模型（时间映射 + classifier guidance 可选）
        self.model_fn = model_wrapper(
            model=self.model,
            noise_schedule=self.noise_schedule,
            time_input_type='0',   # 用 discrete label 训练过
            total_N=1000,
        )

        # 创建 solver
        self.solver = DPM_Solver(self.model_fn, self.noise_schedule)

    def sample(self, shape, steps=50, eps=1e-4, return_all=False):
        """
        生成一个样本（海冰预测图像序列）

        Args:
            shape: (B, C, H, W)，如 [4, 24, 432, 432]
            steps: 采样步数（推荐 10~20）
            eps: 最小时间（建议 1e-4）
        Returns:
            x0: 模型预测的海冰图像（在 x_start 空间）
        """
        # 初始化噪声
        x_T = torch.randn(*shape).to(self.device)

        # 执行采样
        x_0 = self.solver.sample(
            x=x_T,
            steps=steps,
            eps=eps,
            order=2,
