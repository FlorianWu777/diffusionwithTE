...
class PatchEmbed3d(nn.Module):
    def __init__(self, patch_size=(2,3,3), in_chans=1, embed_dim=256):
        super().__init__()
    ...
    def forward(self, x):
        print('PE3D', x.shape)
        x = self.proj(x)
        print('PE3Dafter', x.shape)
        x = x.permute(0,2,3,4,1) # convert to BHWC
        return x


class PatchExpand3d(nn.Module):
    def __init__(self, patch_size=(2,3,3), out_chans=1, embed_dim=256):
        super().__init__()
        self.patch_size = patch_size
...
class AFNOCrossAttentionBlock3d(nn.Module):
    ...
    def __init__(
        ...
        self.total_dim = dim + context_dim
        self.pre_proj = nn.Linear(self.total_dim, self.total_dim)
        self.filter = AFNO3D(self.total_dim, num_blocks, sparsity_threshold, 
            hard_thresholding_fraction) 
        self.mlp = Mlp(
