...
def generate_ensembles(
    ...
    """
    For entire dataset loader, generate one ensemble per sample,
    and finally concatenate to shape: (N, E, T, H, W)
    where E = 1 (GT) + ensemble_size - 1, only for variable C=1 (e.g. siconca/abs)
    """
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(loader):
        ...
        for sample_idx in range(B):
            ...
            # Append ground truth (C=1 only)
            y_single_c1 = y_single[:, 1:2, 1::12, :, :]  # (1, 1, T, H, W)
            preds.append(y_single_c1.squeeze(0).unsqueeze(0).cpu())

            for ens_id in range(1, ensemble_size):  # start from 1
                torch.manual_seed(ens_id)
                with torch.no_grad():
                    pred = lit_module.predict_step((x_single, None), batch_idx=0)

                pred_c1 = pred[:, 1:2, 1::12, :, :]  # (1, 1, T, H, W)
                preds.append(pred_c1.squeeze(0).unsqueeze(0).cpu())

            ensemble_tensor = torch.cat(preds, dim=0)  # (E, 1, T, H, W)
            all_preds.append(ensemble_tensor.unsqueeze(0))  # (1, E, 1, T, H, W)

            del ensemble_tensor, preds, x_single, y_single, pred
            torch.cuda.empty_cache()
            gc.collect()

    # Final shape: (N, E, 1, T, H, W)
    final_tensor = torch.cat(all_preds, dim=0)
    save_path = out_dir / f"ensemble_{split_name}_c1.pt"
    torch.save(final_tensor, save_path)
    print(f"Saved C=1 ensemble to {save_path} → shape: {tuple(final_tensor.shape)}")
