...
# -*- coding: utf-8 -*-
"""
train_seaice_single.py  —  单 Encoder + AFNO + 3D-UNet 季节海冰扩散模型
"""
...
class SeaIceDataModule(pl.LightningDataModule):
    def __init__(self, root_dir, batch_size=4, future_steps=3):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        # CMIP dataset (used for training)
        ...
        # Reanalysis dataset (used for val/test)
        ...
        valid_indices = list(range(train_cutoff, valid_cutoff))
        test_indices = list(range(valid_cutoff, total_len))

        self.reanal_valid_set = Subset(self.reanal_dataset, valid_indices)
        self.reanal_test_set = Subset(self.reanal_dataset, test_indices)

    def train_dataloader(self):
        sampler = DistributedSampler(self.cmip_dataset) if dist.is_initialized() else None
        return DataLoader(
            self.cmip_dataset,
    ...
    def val_dataloader(self):
        sampler = DistributedSampler(self.reanal_valid_set, shuffle=False) if dist.is_initialized() else None
        return DataLoader(
            self.reanal_valid_set,
            batch_size=self.batch_size,
            sampler=sampler,
            num_workers=4,
            pin_memory=True
    ...
    def test_dataloader(self):
        sampler = DistributedSampler(self.reanal_test_set, shuffle=False) if dist.is_initialized() else None
        return DataLoader(
            self.reanal_test_set,
            batch_size=self.batch_size,
            sampler=sampler,
            num_workers=4,
            pin_memory=True
...
def setup_singleenc_model(
        ...
        autoenc_ckpt: str = "/data/wuhaotian/diffusionDemo/model/simpleautoencoder_ftbest.pt",
